...
users.post('/signup', (req, res) => {
  try {
    ...
    pool.query(
      ...
      (error, resultCount) => {
        if (error) {
          res.status(500).send({erroe: error.code, message: error.message});
        } else {
          if (resultCount[0].count > 0) {
            ...
            bcryptjs.hash(password, 10).then((hashedPassword) => {
              ...
              pool.query(query, (error, result) => {
                if (error) {
                  res.status(401).send({erroe: error.code, message: error.message});
                } else {
    ...
    res.status(400).send({erroe: error.code, message: error.message});
  }
});
